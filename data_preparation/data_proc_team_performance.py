...
attendance_proc_df = pd.read_csv('./processed_data/attendance.csv')
...
att_df.loc[:, 'Superbowl winners last year'] = att_df.apply(lambda x: superbowl_winners.loc[(superbowl_winners['year'] == x['year']-1) & (superbowl_winners['team_name'] == x['team_name'])].shape[0], axis=1)
...
attendance_proc_df.loc[:, ['Sb_winners', 'Sb_winners_prev_years', 'Playoff_prev_years', 'Bad_streak', 'Medium_streak', 'Good_streak']] = att_df.loc[:, ['Superbowl winners last year', 'Superbowl winners prev years', 'Playoff finishers prev years', 'Bad_streak', 'Medium_streak', 'Good_streak']]
